...
export function Map() {
  ...
  const [points3D, setPoints3D] = useState<number[][]>([]);
  const [dimensionReductionMethod] = useState('pca');
  const [isLoading, setIsLoading] = useState(true); // 初期ロード中
  const setErrorMessage = useState<string | null>(null)[1]; // エラーメッセージのセッター
  const [hoveredPoint, setHoveredPoint] = useState<number | null>(null);
  const [highlightedPoints, setHighlightedPoints] = useState<number[]>([]); // ランダムに赤く光らせるポイント
  const [searchText, setSearchText] = useState(''); // 検索テキスト
  const [isProcessing, setIsProcessing] = useState(false); // 予言処理中の状態
  
  // APIエンドポイントを環境に応じて設定
  const apiEndpoint = process.env.NEXT_PUBLIC_API_ENDPOINT || '/api/prophet';
  
  const threeContainerRef = useRef<HTMLDivElement>(null);
  // @ts-expect-error - Three.jsの型定義が複雑なため
  ...
  // ベクトルデータの前処理関数
  const preprocessVectors = (vectors: number[][]): number[][] => {
  ...
  const performTSNE = (vectors: number[][]): number[][] => {
    try {
  ...
  const performSimpleUMAP = (vectors: number[][]): number[][] => {
    try {
      // 簡易的なUMAP実装（エラー回避のため）
      // PCAの結果に非線形性を加える
  ...
  const reduceDimensions = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        setErrorMessage(`Error during dimensionality reduction: ${errorMsg}`);
      } finally {
        setIsLoading(false);
      }
    }, 100);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const errorMsg = err instanceof Error ? err.message : String(err);
        setErrorMessage(`Error fetching data: ${errorMsg}`);
  ...
  }, [vectors, dimensionReductionMethod, reduceDimensions]);
  ...
  // Three.jsの初期化
  useEffect(() => {
    // クライアントサイドでのみThree.jsをロード
    if (typeof window !== 'undefined' && points3D.length > 0 && threeContainerRef.current) {
      // 動的にThree.jsをインポート
      const initThreeJS = async () => {
        try {
          ...
          if (threeSceneRef.current) {
            const { scene, renderer, controls } = threeSceneRef.current;
          ...
          // レイキャスティングでホバーしているオブジェクトを検出
          const checkIntersection = () => {
            if (!threeSceneRef.current || !spheresRef.current.length) return;
            
            const { camera: currentCamera, scene: currentScene } = threeSceneRef.current;
            ...
            const intersects = raycasterRef.current.intersectObjects(currentScene.children, true);
          ...
          const animate = () => {
            ...
            spheres.forEach((sphere, index) => {
              if (index === hoveredPoint) {
                ...
                // 完全な予言テキストを表示
                if (sphere.children[0] && sphere.children[0].element) {
                  sphere.children[0].element.classList.remove('hidden');
                }
                // 短いラベルの色を赤に
                if (sphere.children[1] && sphere.children[1].element) {
    ...
    // Cleanup function
    return () => {
      if (threeSceneRef.current && threeSceneRef.current.cleanup) {
        threeSceneRef.current.cleanup();
      }
    };
  }, [points3D, highlightedPoints, hoveredPoint]);
    ...
    <div className="h-screen w-full bg-black flex flex-col overflow-hidden">
      <main className="flex-1 flex flex-col">
        ...
        <div className="flex-1 relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              {isLoading ? (
                <div className="flex flex-col items-center">
                  ...
                  <div className="text-white text-xl font-bold">ベクトルデータを読み込み中...</div>
                </div>
              ) : (
          ...
          {/* 情報オーバーレイ */}
          {points3D.length > 0 && !isLoading && (
            <div className="absolute top-4 left-4 bg-black bg-opacity-70 text-white p-3 rounded-md text-sm">
